...
########################################################### Retrieval ##########################################################
################################# find/findOr/findOrFail/first/firstOr/firstOrFail/all/get/select/addSelect ###########################################
...
Route::get('delete', function () {
    ...
    // $user = User::find([13, 14])->each->delete();// Return all of records data after deleting
    //// ==
    // $user = User::whereIn('id', [5, 255, 265])->delete(); // Return the number of deleted records->integer->1
    //// ==
    // $user = User::destroy([602, 5060, 600]); // Return the number of deleted records->integer->0

    // User::truncate(); //Delete all of the data
    // dd($user);


});

################################## onlyTrashed() / withTrashed() / restore() / delete() / forceDelete() ##########################################################
Route::get('/softDelete', function () {
    // in the model use SoftDelete
    // User::find(5)->delete(); //set the time in deleted_at
    // $user = User::onlyTrashed()->get();
    // $user = User::withTrashed()->get(); // get all (both trashed and untrashed)
    // $user = User::find(5)->restore(); // deleted_at = null
    // $user = User::find(5)->forceDelete();// Delete from the DB

    // dd($user);
});

#################################Prune# prunable() is a must / pruning() optional ##########################################################
########not work with hooks##MassPrune# prunable() is a must / not work with pruning()  ####################################################
Route::get('prune', function () {
    // Artisan::call('model:prune', ['--model' => 'App\Model\User', '--pretend' => true]);
    Artisan::call('model:prune');
});

################################## replicate() = duplicate an instance of data ##########################################################
Route::get('replicate', function () {
    ...
    $admin = $users->replicate(['wallet'])->fill([
        'name' => 'replicate admin',
